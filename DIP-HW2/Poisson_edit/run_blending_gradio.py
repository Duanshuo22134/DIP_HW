...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    laplacian_kernel = laplacian_kernel.repeat(3, 1, 1, 1)  # Shape: (3, 1, 3, 3)
    
    laplacian_fg = F.conv2d(
        foreground_img,
        laplacian_kernel,
        padding=0,
        groups=3
    ...
    mask = (foreground_mask * background_mask)[:, :, 1:-1, 1:-1]

    loss = (torch.abs(laplacian_fg - laplacian_blended) * mask).mean()
    return loss
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    background_polygon_points = foreground_polygon_points + np.array([dx, dy]).reshape(1, 2)
    ...
    # Move the foreground image and mask into the background coordinate system so
    # the Laplacian is computed on tensors with the same spatial size.
    shifted_foreground_np = np.zeros_like(background_np)
    shifted_foreground_mask = np.zeros(background_mask.shape, dtype=np.uint8)
    ...
    src_x0 = max(0, -dx)
    src_y0 = max(0, -dy)
    src_x1 = min(fg_w, bg_w - dx)
    src_y1 = min(fg_h, bg_h - dy)
    ...
    shifted_foreground_np[dst_y0:dst_y1, dst_x0:dst_x1] = foreground_np[src_y0:src_y1, src_x0:src_x1]
    shifted_foreground_mask[dst_y0:dst_y1, dst_x0:dst_x1] = foreground_mask[src_y0:src_y1, src_x0:src_x1]
    shifted_foreground_mask = np.minimum(shifted_foreground_mask, background_mask)

    # Convert numpy arrays to torch tensors
    device = 'cuda:0' if torch.cuda.is_available() else 'cpu'  # Using CPU will be slow
    fg_img_tensor = torch.from_numpy(shifted_foreground_np).to(device).permute(2, 0, 1).unsqueeze(0).float() / 255.
    bg_img_tensor = torch.from_numpy(background_np).to(device).permute(2, 0, 1).unsqueeze(0).float() / 255.
    fg_mask_tensor = torch.from_numpy(shifted_foreground_mask).to(device).unsqueeze(0).unsqueeze(0).float() / 255.
